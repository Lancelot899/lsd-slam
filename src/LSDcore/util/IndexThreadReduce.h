...
public:
	inline IndexThreadReduce()
	{
		...
		maxIndex = 0;
		stepSize = 1;
		callPerIndex = boost::bind(&IndexThreadReduce::callPerIndexDefault, this, _1, _2);
	...
	inline void reduce(boost::function<void(int,int)> callPerIndex, int first, int end, int stepSize = 0)
	{
		...
		nextIndex = 0;
		maxIndex = 0;
		this->callPerIndex = boost::bind(&IndexThreadReduce::callPerIndexDefault, this, _1, _2);
...
private:
	...
	void callPerIndexDefault(int, int)
	{
